...
export class Authorizer<User = unknown, Data = unknown> {
	...
	async authorize(
		...
		const user = await this.authenticator.isAuthenticated(args.event);
		...
		for (const rule of [...this.rules, ...rules]) {
			...
			// @ts-expect-error failureRedirect is a string if raise is redirect
			if (raise === 'redirect') throw redirect(307, failureRedirect);
			if (raise === 'response') {
				if (!rule.name) throw json({ message: 'Forbidden' }, { status: 403 });
				throw json({ message: `Forbidden by policy ${rule.name}` }, { status: 403 });
			}
			if (!rule.name) throw new AuthorizationError('Forbidden.');
			throw new AuthorizationError(`Forbidden by policy ${rule.name}`);
		}
